  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-slate-400">Resumen general y reportes exportables de la competencia.</p>
      </div>
      ...
      <section className="rounded-xl border border-slate-100 bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
          <h2 className="font-bold text-slate-900">Participantes por institución</h2>
          <button
            type="button"
            ...
            className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 px-3 py-1.5 text-sm font-medium text-slate-600 transition-colors hover:bg-slate-50 disabled:opacity-40"
          >
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-50">
              {participantesQ.isLoading ? (
                ...
                participantes.map((p) => (
                  <tr key={p.institucion_id} className="hover:bg-slate-50/60">
                    <td className="px-6 py-3 font-medium text-slate-900">{p.institucion}</td>
                    <td className="px-4 py-3 text-center text-slate-600">{p.equipos}</td>
                    <td className="px-4 py-3 text-center font-semibold text-slate-900">{p.atletas}</td>
                  </tr>
                ))
              )}
            </tbody>
      ...
      <section className="rounded-xl border border-slate-100 bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
          <h2 className="font-bold text-slate-900">Equipos por disciplina</h2>
          <button
            type="button"
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-50">
              {equiposQ.isLoading ? (
                <tr><td colSpan={2} className="px-6 py-8 text-center text-slate-400">Cargando…</td></tr>
              ) : equipos.length === 0 ? (
                <tr><td colSpan={2} className="px-6 py-8 text-center text-slate-400">Sin datos</td></tr>
              ) : (
                equipos.map((e) => (
                  <tr key={e.deporte_id} className="hover:bg-slate-50/60">
                    <td className="px-6 py-3 font-medium text-slate-900">{e.deporte}</td>
                    <td className="px-4 py-3 text-center font-semibold text-slate-900">{e.equipos}</td>
                  </tr>
                ))
              )}
            </tbody>
