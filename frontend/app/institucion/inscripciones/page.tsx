    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-0.5">Inscribe tus equipos en los torneos disponibles.</p>
        </div>
      ...
      {torneosActivos.length > 0 && (
        <div>
          <p className="text-xs font-semibold tracking-widest text-slate-400 uppercase mb-3">Torneos disponibles</p>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {torneosActivos.map(t => (
              <div key={t.id} className="bg-white border border-slate-100 rounded-xl p-4 shadow-sm">
                <div className="flex items-start justify-between">
                  <div>
                    <p className="text-sm font-semibold text-slate-900">{t.nombre}</p>
                    <p className="text-xs text-slate-400 mt-0.5">{nombreDeporte(t.deporte_id)} · {t.temporada}</p>
                  </div>
      ...
      <div className="bg-white border border-slate-100 rounded-xl shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-50 flex items-center justify-between">
          <h2 className="text-sm font-semibold text-slate-900">Mis inscripciones</h2>
          <span className="text-xs text-slate-400">{inscripciones.length} total</span>
        </div>
        {cargando ? (
          ...
          <div className="flex flex-col items-center justify-center h-40 text-slate-300">
            ...
            <p className="text-sm text-slate-400">Sin inscripciones aún</p>
          </div>
        ) : (
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
            <h2 className="text-lg font-bold text-slate-900 mb-5">Inscribir equipo</h2>
