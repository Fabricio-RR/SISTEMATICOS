...
const ESTADO_BADGE: Record<string, string> = {
  ...
  aprobado:  "bg-green-50 text-green-700 border border-green-100",
...
export default function InstitucionEquiposPage() {
  ...
  const [formEquipo, setFormEquipo] = useState({ nombre_equipo: "", deporte_id: 0 });
  const [guardandoEquipo, setGuardandoEquipo] = useState(false);
  const [errorEquipoForm, setErrorEquipoForm] = useState("");
  ...
  const cargar = useCallback(async () => {
    ...
    setError("");
    try {
      const [currentUser, eq, dep, torn] = await Promise.all([
        api.me(),
        api.getEquipos(),
        api.getDeportes(),
        api.getTorneos(),
      ...
      // Filtrar solo equipos de mi institución
      const propios = eq.filter((e) => e.institucion_id === currentUser.institucion_id);
      setEquipos(propios);
    } catch {
      setError("No se pudo cargar la información de tus equipos.");
    } finally {
      setCargando(false);
    }
  }, []);
  ...
  async function handleCreateEquipo(e: React.FormEvent) {
    ...
    setSuccess("");
    if (!me) return;
    if (!formEquipo.nombre_equipo || !formEquipo.deporte_id) {
      setErrorEquipoForm("Selecciona un deporte e ingresa un nombre para el equipo.");
      return;
    }
    if (formEquipo.nombre_equipo.length > 50) {
      setErrorEquipoForm("El nombre del equipo no puede tener más de 50 caracteres.");
      return;
    }
    ...
    try {
      await api.createEquipo({
        ...
        institucion_id: me.institucion_id as number,
        deporte_id: formEquipo.deporte_id,
      });
      setModalNuevoEquipo(false);
      setFormEquipo({ nombre_equipo: "", deporte_id: 0 });
      setSuccess("Solicitud de creación de equipo registrada. El administrador deberá aprobarla.");
      await cargar();
    } catch (err) {
      setErrorEquipoForm(err instanceof Error ? err.message : "Error al registrar el equipo.");
    } finally {
  ...
  async function handleDeleteAtleta(atleta: AtletaJugador) {
    if (!confirm(`¿Eliminar al atleta "${atleta.nombre_completo}" de este equipo?`)) return;
    try {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-sm font-semibold rounded-xl transition shadow-sm"
          >
      ...
      {error && (
        <div className="flex items-start gap-2 bg-red-50 border border-red-200 rounded-2xl px-4 py-3.5 text-sm text-red-600">
      ...
      {success && (
        <div className="flex items-center gap-2 bg-green-50 border border-green-200 rounded-2xl px-4 py-3.5 text-sm text-green-700">
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            {eq.estado === "aprobado" ? (
                              <button
                                ...
                                className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-red-600 hover:bg-red-700 text-white text-xs font-semibold rounded-lg transition shadow-sm animate-fade-in"
                              >
                              ...
                              <span className="text-xs text-amber-600 bg-amber-50 px-2 py-1 rounded border border-amber-100 font-medium">
                                El equipo debe ser aprobado por el administrador para poder registrar atletas
                              </span>
                            )}
                          </div>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 border border-slate-100">
            ...
            <form onSubmit={handleCreateEquipo} className="space-y-4">
              ...
              {errorEquipoForm && <p className="text-sm text-red-600 bg-red-50 px-4 py-2.5 rounded-lg border border-red-100">{errorEquipoForm}</p>}
              <div className="flex gap-3 pt-2">
                ...
                >
                  {guardandoEquipo ? "Guardando..." : "Enviar Solicitud"}
                </button>
              </div>
            </form>
          </div>
